...
import { Link, Link as LinkR, NavLink, useNavigate } from "react-router-dom";
  ...
  return (
    <Nav>
        <NavContainer>
          ...
          {currentUser && (
            <>
            <Link to ="/userProfile" style = {{ textDecoration:'none'}}><Avatar src = {currentUser?.img}>{currentUser?.name[0]}</Avatar></Link>
            </>
          )}
            </MobileIcons>
            ...
            {isOpen && (
                ...
                <Navlink to ='/myorders' onClick={() =>setIsOpen(false)}>Orders</Navlink>
                ...
                <div style ={{
                  ...
                  <Button text ="Sign Up" small outlined  onClick = {()=> setOpenAuth(true)}/>
            ...
            <ButtonContainer>
            <NavLink to ='/search'>
            <SearchRounded sx={{ color: "inherit", fontSize: "30px" }}/> </NavLink>
            {currentUser ? (
              <>
              <NavLink to ='/favourite'>

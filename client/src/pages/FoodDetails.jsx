...
const removeFavourite = async () => {
  ...
  try {
    await deleteFromFavourite(token, { productId: id });
    setFavorite(false); // ✅ instantly update state
...
const addFavourite = async () => {
  ...
  try {
    await addToFavourite(token, { productId: id });
    setFavorite(true); // ✅ instantly update state
...
const checkFavourite = async () => {
  ...
  try {
    ...
    console.log("getFavourite response:", res.data);

    const favourites = res.data?.favouriteProducts || [];

    // if (Array.isArray(res.data)) {
    //   favourites = res.data;
    // } else if (Array.isArray(res.data?.favourites)) {
    //   favourites = res.data.favourites;
    // } else if (Array.isArray(res.data?.data)) {
    //   favourites = res.data.data;
    // }

    const isFavorite = favourites.some((fav) => fav._id === id);
    setFavorite(isFavorite);
  } catch (err) {
    dispatch(
      openSnackbar({
